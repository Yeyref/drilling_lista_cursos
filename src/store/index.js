...
export default new Vuex.Store({
  ...
  getters: {
    getProductById: (state) => (id) => {
      return state.cursos.find((prod) => prod.id === id);
    },
  },
  mutations: {
    ADD_PRODUCT: (state, product) => {
      product.id = Math.floor(Math.random() * 1000);
      state.cursos.push(product);
    },
    ...
    SET_ID_PRODUCT_ID: (state, id) => {
      state.id_product_id = id;
    },
    EDIT_PRODUCT: (state, product) => {
      let index = state.cursos.findIndex((prod) => prod.id === product.id);
      Vue.set(state.cursos, index, product);
      state.id_product_id = null;
    },
  },
  actions: {
